...
def test_card_matching():
    top = Card("Red", "5")
    assert Card("Red", "8").matches(top, "Red")
    assert Card("Blue", "5").matches(top, "Red")
    assert Card("Wild", "Wild").matches(top, "Red")
    assert not Card("Blue", "8").matches(top, "Red")
...
def test_dynamic_point_uses_remaining_card_value():
    ...
    assert calc_dynamic_point_delta(3, 1, 0, 47) == 22
    assert calc_dynamic_point_delta(3, 3, 42, 0) == -12
